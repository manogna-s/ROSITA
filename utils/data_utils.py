...
def get_desired_data(args, te_transforms, tesize=10000):
    ...
    if args.desired == 'cifar10':
        ...
        desired_dataset = CustomImageFolder(corruption_dir_path, te_transforms, tesize=tesize)
    ...
    elif args.desired in ["DN-Clipart", "DN-Painting", "DN-Sketch"] :
            ...
            img_list = {"Clipart":'clipart', "Painting":'painting', "Sketch":'sketch'}
    ...
    elif args.desired == 'CCC':
        def identity(x):
            return x
        domain_name = "baseline_20_transition+speed_1000_seed_44"
        url = f'https://mlcloud.uni-tuebingen.de:7443/datasets/CCC/{domain_name}/serial_{{00000..99999}}.tar'
        desired_dataset = (wds.WebDataset(url)
                    .decode("pil")
                    .to_tuple("input.jpg", "output.cls")
                    .map_tuple(te_transforms, identity)
                    .with_length(150000)
            )
        print(len(desired_dataset))
        imagenet_classes = read_classnames(f'datasets/imagenet_classnames.txt')
        data_dict['ID_classes'] = imagenet_classes
        data_dict['N_classes'] = len(data_dict['ID_classes'])
