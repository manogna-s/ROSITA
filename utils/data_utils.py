...
def get_weak_ood_data(args, te_transforms, tesize=10000):
    ...
    if args.weak_OOD == 'cifar10OOD':
        ...
        weak_ood_dataset = CustomImageFolder(corruption_dir_path, te_transforms, tesize=tesize)
    ...
    elif args.weak_OOD in ["QuickdrawOOD", "ClipartOOD", "PaintingOOD", "SketchOOD", "InfographOOD"] :
            ...
            weak_ood_dataset = DomainNet(root= f'{args.dataroot}/domainnet', label_files=f'{args.dataroot}/domainnet/{img_list[args.weak_OOD]}_test.txt' , transform=te_transforms, tesize=50000)
...
def get_strong_ood_data(args, te_transforms, tesize=10000):
    ...
    return data_dict, strong_ood_dataset


def prepare_ood_test_data(args, te_transforms):

    weak_ood_data_dict, weak_ood_dataset = get_weak_ood_data(args, te_transforms, tesize=args.tesize)
    strong_ood_data_dict, strong_ood_dataset = get_strong_ood_data(args, te_transforms, tesize=args.tesize)
    data_dict = dict(list(weak_ood_data_dict.items()) + list(strong_ood_data_dict.items()))
