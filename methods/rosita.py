...
def ROSITA(args, model, data_loader, classifiers):
    ...
    for i, (image, gt) in tqdm(enumerate(data_loader)):
        ...
        if W_pred[0].item() and maxlogit_tta.item()>stats['mu1']:
            proto_bank['W'].append(image_features_raw.detach())
            proto_bank['W'] = proto_bank['W'][-C * args.k_p:]
        ...
        if args.loss_simclr and len(proto_bank['W'])>args.k_p and len(proto_bank['S'])>args.k_p:  #i>100:
            if maxlogit_tta.item()>stats['mu1'] or maxlogit_tta.item()<stats['mu0']:
            
                if W_pred[0].item():
                    n_p = args.k_p
                    pos_features = torch.vstack(proto_bank['W']) 
                    neg_features = torch.vstack(proto_bank['S'])
                    ...
                    pos_features = torch.vstack(proto_bank['S'])
                    neg_features = torch.vstack(proto_bank['W']) 
